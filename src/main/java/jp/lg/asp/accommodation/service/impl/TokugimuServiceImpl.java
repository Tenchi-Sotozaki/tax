...
public class TokugimuServiceImpl implements TokugimuService {
	...
	public List<TokugimuListItem> search(CollectorSearchForm form) {
		// データベースから検索
	...
	public TokugimuForm getTokugimuById(Long id) {
		...
		form.setId(id);
		form.setTokugimuName(atena.getName());
		// 他のフィールドは必要に応じて追加
	...
	public List<FacilityDto> getFacilities(String obligorId) {
		...
		String name = getTokugimuName(obligorId);
		return List.of(
	...
	public TaxManagerForm buildTaxManagerForm(Long collectorId) {
		...
		form.setObligorName(tokugimu.getTokugimuName());
		return form;
	}
	...
	public void register(TokugimuForm form) {
		// 実装予定
		log.info("特別徴収義務者登録: {}", form.getTokugimuName());
	}
	...
	public void update(Long id, TokugimuForm form) {
		// 実装予定
		log.info("特別徴収義務者更新: id={}, name={}", id, form.getTokugimuName());
	}
