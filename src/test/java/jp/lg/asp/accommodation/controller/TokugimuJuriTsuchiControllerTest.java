...
/**
 * 特別徴収義務者申請受理通知 Controller（ACCOMMODATION_TAX-349）の単体テスト。
 *
...
class TokugimuJuriTsuchiControllerTest {
    ...
    private TokugimuJuriTsuchiDto info() {
    ...
    void index_セッションに指定番号があればサービスから取得した情報を表示する() {
        when(tokugimuJuriTsuchiService.getTokugimuInfo(SHITEI_NO)).thenReturn(info());
        Model model = new ExtendedModelMap();

        String view = controller.index(sessionWith(SHITEI_NO), model);

        assertThat(view).isEqualTo("reports/tokugimuJuriTsuchi");
        TokugimuJuriTsuchiDto dto = (TokugimuJuriTsuchiDto) model.asMap().get("dto");
