...
/**
 * 合算申告納入承認通知書 Controller（ACCOMMODATION_TAX-356）の単体テスト。
 *
...
class GassanNonyuTsuchiControllerTest {
    ...
    @Mock GassanNonyuTsuchiService gassanNonyuTsuchiService;
    @Mock GassanNonyuTsuchiReportsService reportsService;
    @Mock ScreenAccessChecker accessChecker;

    @InjectMocks GassanNonyuTsuchiController controller;
    ...
    private GassanNonyuTsuchiDto info() {
    ...
    void index_指定番号が無ければ特別徴収義務者指定モーダルを表示する() {
        ...
        assertThat(view).isEqualTo("reports/gassanNonyuTsuchi");
        assertThat(model.asMap()).containsEntry("showShiteiGassanModal", true);
        verify(gassanNonyuTsuchiService, never()).getGassanNonyuTsuchiInfo(any());
    }

    @Test
    void index_セッションに指定番号があればサービスから取得した情報を表示する() {
        when(gassanNonyuTsuchiService.getGassanNonyuTsuchiInfo(SHITEI_NO)).thenReturn(info());
        Model model = new ExtendedModelMap();
    ...
    void index_サービスがnullを返しても発行日が本日で画面を返す() {
        when(gassanNonyuTsuchiService.getGassanNonyuTsuchiInfo(SHITEI_NO)).thenReturn(null);
        Model model = new ExtendedModelMap();

        String view = controller.index(sessionWith(SHITEI_NO), model);

        assertThat(view).isEqualTo("reports/gassanNonyuTsuchi");
        GassanNonyuTsuchiDto dto = (GassanNonyuTsuchiDto) model.asMap().get("dto");
