...
class JichitaiConfigDtoTest {
	...
	void 自治体コードが未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("jichitaiCd") &&
	...
	void 自治体種別名が未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("kbnName") &&
	...
	void 年度開始月が未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("nendoStMonth") &&
	...
	void デフォルト納税周期が未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("nozeiShuki") &&
	...
	void 指定番号が未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("shiteiStChar") &&
	...
	void 合算指定番号が未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("gassanStChar") &&
	...
	void 宛名番号がnullの場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("atenaStNo") &&
	...
	void 自治体識別名がnullの場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("param") &&
	...
	void ユーザーIDが未入力の場合_エラーメッセージを返す() {
		...
		assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("userId") &&
				v.getMessage().equals("初期ユーザーIDは必須です。"));
	}
}
