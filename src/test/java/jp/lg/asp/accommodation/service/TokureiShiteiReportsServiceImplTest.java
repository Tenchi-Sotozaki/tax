...
/**
 * 納入申告書の提出期限等の特例適用者指定通知書 PDF生成（ACCOMMODATION_TAX-351）の単体テスト。
 *
...
class TokureiShiteiReportsServiceImplTest {
    ...
    private final TokureiShiteiReportsServiceImpl service = new TokureiShiteiReportsServiceImpl();
    ...
    /** 全項目を埋めた入力 */
    private TokureiShiteiDto fullDto() {
    ...
    void generateTsuchiPdf_全項目nullでも例外にならずPDFが生成される() {
        assertPdf(service.generateTsuchiPdf(new TokureiShiteiDto()));
    }

    @Test
    void generateTsuchiPdf_公印が空配列でも例外にならずPDFが生成される() {
        TokureiShiteiDto dto = fullDto();
