...
/**
 * 納入申告書の提出期限等の特例適用者指定取消通知書 PDF生成（ACCOMMODATION_TAX-353）の単体テスト。
 *
...
class TokureiShiteiCancelReportsServiceImplTest {
    ...
    private final TokureiShiteiCancelReportsServiceImpl service = new TokureiShiteiCancelReportsServiceImpl();
    ...
    void generateTsuchiPdf_全項目nullでも例外にならずPDFが生成される() {
        assertPdf(service.generateTsuchiPdf(new TokureiShiteiCancelDto()));
    }

    @Test
    void generateTsuchiPdf_公印が空配列でも例外にならずPDFが生成される() {
        TokureiShiteiCancelDto dto = fullDto();
