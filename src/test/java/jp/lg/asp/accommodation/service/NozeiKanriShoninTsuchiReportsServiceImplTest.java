...
/**
 * 納税管理人承認(不承認)通知書 PDF生成（ACCOMMODATION_TAX-350）の単体テスト。
 *
...
class NozeiKanriShoninTsuchiReportsServiceImplTest {
    ...
    private final NozeiKanriShoninTsuchiReportsServiceImpl service = new NozeiKanriShoninTsuchiReportsServiceImpl();
    ...
    /** 全項目を埋めた入力 */
    private NozeiKanriShoninTsuchiDto fullDto() {
        ...
        dto.setNozeiKanriJusho("札幌市北区北10条西3丁目");
        dto.setNozeiKanriName("山田太郎");
        dto.setRiyu("テスト用の理由");
    ...
    void generateTsuchiPdf_全項目nullでも例外にならずPDFが生成される() {
        assertPdf(service.generateTsuchiPdf(new NozeiKanriShoninTsuchiDto()));
    }

    @Test
    void generateTsuchiPdf_公印が空配列でも例外にならずPDFが生成される() {
        NozeiKanriShoninTsuchiDto dto = fullDto();
