...
class CommaFormattedStringToLongConverterTest {

    private CommaFormattedStringToLongConverter converter;

    @BeforeEach
    void setUp() {
        converter = new CommaFormattedStringToLongConverter();
    }
    ...
    void convert_カンマあり数値を変換できる() {
        assertThat(converter.convert("1,234,567")).isEqualTo(1234567L);
    }
    ...
    @Test
    void convert_Long範囲の大きな数値を変換できる() {
        assertThat(converter.convert("9,999,999,999")).isEqualTo(9999999999L);
    }

    @Test
    void convert_負の数を変換できる() {
        assertThat(converter.convert("-1000")).isEqualTo(-1000L);
    }
